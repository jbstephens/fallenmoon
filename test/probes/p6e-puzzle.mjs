...
/* THE TONE IS THE PLAYER'S CLOCK, NOT THE HARNESS'S. This probe used to
   poll for the tone in a 120ms loop, then gate, then waitTicks, then take a
   screenshot — and only then set off. Headless runs the sim at ~20fps, so
   those setup calls quietly ate ~14 of the tone's 15 seconds and the walk
   arrived at a door that had already shut. Ring it, and leave AT ONCE;
   assert and photograph while the legs are moving. */
await api.eval('__fakePad.press(0)');
await api.waitFor('__fm.fToneT > 0', 15000, 'the great bell tolls');
await api.eval('__fakePad.press()');
await api.eval(`__fmBot.tol = 1.2; __fmBot.target = [-1468, -444]; __fmBot.sprint(true); 0`);
const tone = await api.eval('__fm.fToneT');
gate(tone > 8, 'HOLD: holding ✕ tolls the bell and a long tone sustains', 'toneT=' + (+tone).toFixed(1));
gate(await api.eval('__fm.fDoorK > 0.4'), 'HOLD: the door is visibly LIFTING while the tone lives');
await api.shot('verb-3-hold');

await api.waitFor('__fm.fGlyph3 === true', 22000, 'walked the door in time').catch(() => {});
